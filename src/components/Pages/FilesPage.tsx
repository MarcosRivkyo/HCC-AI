...
import { FiChevronDown, FiChevronRight, FiFileText, FiImage, FiUpload, FiTrash2, FiEye } from "react-icons/fi";
...
const FilesPage: React.FC = () => {
  ...
  const handleUpload = (folderPath: string, event: React.ChangeEvent<HTMLInputElement>) => {
...
<div className="relative z-50">
  ...
  {!showAssistant && (
    ...
    <main className="w-full max-w-7xl mx-auto px-6 pt-20 flex-grow">
        <h2 className="text-4xl mt-8 font-bold text-center text-gray-800 dark:text-white mb-16 flex items-center justify-center gap-2"> Archivos</h2>
        <div className="space-y-6">
          {Object.entries(groupedFiles).map(([folderName, files]) => {
            const isOpen = openFolders[folderName];
            return (
              <div key={folderName} className="bg-white dark:bg-gray-800 border rounded-xl shadow-md">
                <div className="flex items-center justify-between px-5 py-4 text-lg font-semibold text-gray-800 dark:text-white bg-gray-50 dark:bg-gray-700 rounded-t-xl">
                  <div className="flex items-center gap-3 capitalize">
                    {folderName === "informes" ? <FiFileText size={20} /> : <FiImage size={20} />}
                    {folderName}
                  </div>
                  <div className="flex items-center gap-3">
                    <label className="cursor-pointer flex items-center gap-1 text-blue-600 dark:text-blue-400 hover:underline">
                      <FiUpload />
                      <input
                        type="file"
                        hidden
                        onChange={(e) => {
                          const path = folderName === "informes" ? userData?.documentFolder : `${userData?.imageFolder}${folderName}/`;
                          handleUpload(path, e);
                        }}
                      />
                    </label>
                ...
                {isOpen && (
                  <div className="px-5 pb-6 pt-2 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
                    {files.map((file, idx) => {
                      const folderPath = folderName === "informes" ? userData?.documentFolder : `${userData?.imageFolder}${folderName}/`;
                      return (
                        <div key={idx} className="relative bg-gray-100 dark:bg-gray-700 hover:shadow-xl p-4 rounded-lg">
                          <button
                            onClick={() => setFileToDelete({ folderPath, fileName: file.name })}
                            className="absolute top-2 right-2 text-red-500 hover:text-red-700"
                            title="Eliminar"
                          >
                            <FiTrash2 size={18} />
                          </button>
                          <p className="font-semibold text-gray-800 dark:text-gray-100 text-sm truncate mb-2">{file.name}</p>
                          {folderName === "informes" ? (
                            <iframe src={`${file.url}#zoom=30`} title={file.name} className="w-full h-80 rounded border" />
                          ) : file.url.endsWith(".pdf") ? (
                            <iframe src={`${file.url}#zoom=50`} title={file.name} className="w-full h-48 rounded border" />
                          ) : (
                            <img src={file.url} alt={file.name} className="w-full h-48 object-cover rounded shadow-sm" />
                          )}
                          <a
                            href={file.url}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-sm text-blue-600 dark:text-blue-400 flex items-center gap-1 mt-3"
                          >
                            <FiEye /> Ver
                          </a>
                        </div>
      ...
      {fileToDelete && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-xl max-w-sm w-full">
            <h2 className="text-lg font-bold text-gray-800 dark:text-white mb-4">Confirmar eliminación</h2>
            <p className="text-sm text-gray-600 dark:text-gray-300 mb-6">
              ¿Estás seguro de que deseas eliminar <strong>{fileToDelete.fileName}</strong>? Esta acción no se puede deshacer.
            </p>
